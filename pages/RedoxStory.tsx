...
const RedoxStory: React.FC<PageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-brand-50 flex flex-col font-sans">
      <div className="max-w-4xl mx-auto w-full px-4 py-8 flex-grow space-y-8">
        ...
        <div className="relative bg-white rounded-3xl p-8 shadow-xl shadow-orange-100/50 border-2 border-orange-100 overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="prose prose-orange max-w-none text-slate-700 leading-relaxed text-lg font-serif">
              <p>Di sebuah kerajaan logam yang kokoh, hiduplah <strong>Pangeran Besi (Fe)</strong>...</p>
              <p>Suatu hari, datanglah <strong>Penyihir Oksigen (O₂)</strong>...</p>
              <p>"Wahai Pangeran," kata Penyihir Oksigen, "Berikanlah permatamu..." Pangeran Besi pun <strong>melepaskan</strong> permata elektronnya.</p>
              <p>Tubuhnya berubah menjadi kemerahan dan rapuh. Ia telah berubah menjadi sosok baru: <strong>Karat (Fe₂O₃)</strong>.</p>
            </div>
        ...
        {/* Concept Reveal */}
        {showExplanation && (
          <div className="bg-slate-800 text-white rounded-3xl p-6 md:p-8 shadow-lg">
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
              <p className="text-slate-600 mb-4">Bayangkan sebuah jembatan besi di dekat pantai...</p>
            </div>
                ...
                <div key={q.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    {q.options.map((opt) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 rounded-lg text-sm transition-all border
                          ${answers[q.id] === opt.id 
                            ? (opt.isCorrect ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800')
                      ...
                      >
                        <div className="flex justify-between items-center">
                          {opt.text}
                          {answers[q.id] === opt.id && (opt.isCorrect ? <CheckCircle2 size={16} /> : <XCircle size={16} />)}
                        </div>
                      </button>
                    ))}
                  </div>
                  {feedback[q.id] !== undefined && (
                    <p className={`text-xs mt-2 font-medium ${feedback[q.id] ? 'text-green-600' : 'text-red-500'}`}>
                      {feedback[q.id] ? "Benar!" : "Kurang tepat."}
                    </p>
                  )}
                </div>
              ))}
