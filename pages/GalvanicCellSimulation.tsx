...
// --- CONSTANTS ---
const ELECTRODES: Record<string, { name: string; E0: number; color: string }> = {
...
const calculateVoltage = (anodeKey: string, cathodeKey: string, concAnode: number, concCathode: number) => {
  ...
  return Math.max(0, parseFloat((E_cell_std - correction).toFixed(3)));
};

const GalvanicCellSimulation: React.FC<PageProps> = ({ onNavigate }) => {
  // --- STATE ---
    ...
    if (isRunning) {
      ...
      timerRef.current = setInterval(() => {
        setTime(prev => {
          ...
          setGraphData(curr => {
            const newData = [...curr, { time: newTime, voltage }];
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-brand-100">
          ...
          <div className="flex items-center gap-2 bg-brand-100 px-4 py-2 rounded-xl">
            ...
            <span className="text-2xl font-mono font-bold text-brand-700">{voltage.toFixed(3)} V</span>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-4 relative overflow-hidden min-h-[300px] flex items-center justify-center">
               <svg viewBox="0 0 600 350" className="w-full h-full max-w-lg">
                  ...
                  <path d="M 150 120 V 50 H 450 V 120" fill="none" stroke="#475569" strokeWidth="4" strokeLinecap="round" />
                  <circle cx="300" cy="50" r="25" fill="#f8fafc" stroke="#475569" strokeWidth="3" />
                  <text x="300" y="55" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#334155">{voltage.toFixed(2)}V</text>

                  <path d="M 230 150 V 100 Q 300 60 370 100 V 150" fill="none" stroke="#e2e8f0" strokeWidth="24" strokeLinecap="round" className="opacity-80" />
                  ...
                  <text x="150" y="300" textAnchor="middle" fontSize="16" fontWeight="bold" fill="#334155">- {anodeKey}</text>
                  ...
                  <text x="450" y="300" textAnchor="middle" fontSize="16" fontWeight="bold" fill="#334155">+ {cathodeKey}</text>

                  {/* Electrons Animation */}
                  {[0, 0.6, 1.3].map((delay, i) => (
                    <circle key={i} r="6" fill="#fbbf24" stroke="#d97706" strokeWidth="1" opacity={isRunning ? 1 : 0}>
                      {isRunning && (
                        <animateMotion 
                          dur="2s" 
                          begin={`${delay}s`}
            ...
            <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100 flex-grow min-h-[250px] flex flex-col">
              <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2">
                <Zap size={18} className="text-brand-500" /> Grafik Potensial Sel
              </h3>
              <div className="flex-grow w-full h-full">
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={graphData}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                    <XAxis dataKey="time" label={{ value: 'Waktu (s)', position: 'insideBottomRight', offset: -5 }} stroke="#94a3b8" tick={{fontSize: 12}} />
                    <YAxis domain={[0, 'auto']} label={{ value: 'Volt (V)', angle: -90, position: 'insideLeft' }} stroke="#94a3b8" tick={{fontSize: 12}} />
                    <Tooltip />
                    <Line type="monotone" dataKey="voltage" stroke="#f97316" strokeWidth={3} dot={false} isAnimationActive={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
